...
do
	...
	./NN --method "$METHOD" --testing-mode set --num-topics $NUM_TOPICS --num-iterations $NUM_ITER --output-model $DIR_NAME --num-top-words 15 --training-file ../test_data/"$DATASET".train --testing-file ../test_data/"$DATASET".test | tee -a $DIR_NAME/log.txt
done
done
